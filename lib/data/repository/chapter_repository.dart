...
class ChapterRepo {
  ...
  Future<List<Image>> readChapterNext({required String comicId,required int numerical})async{
    final chapter = await HandleDatabase.readNextChapterByNumberic(comicId: comicId, numerical: numerical + 1);
    if (chapter != null) {
      // print('${chapter.id} -------------------------------------------------------------------------');
  ...
  Future<int> readChapterNumberic({required String chapterId})async{
    Chapter? chapter = await HandleDatabase.readChapterByIDFromDB(id: chapterId);
    if (chapter != null) {
